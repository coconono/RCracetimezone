...
class CarLearningState:
    ...
    def adapt(self, profile: CarBehaviorProfile, memory: CarRaceMemory) -> None:
        ...
        speed_push = 1.0 + (profile.speed_priority + profile.avoid_slowdown_priority) * 0.03
        self.target_speed_bias = min(1.35, max(0.75, speed_push))

        safety_penalty = (avg_damage / 100.0) * 0.45 + min(1.0, avg_hits / 8.0) * 0.35
        recovery = (1.0 - completed_ratio) * 0.25
        self.safety_bias = min(1.45, max(0.7, 1.0 + safety_penalty + recovery))

        aggression_base = 1.0 + profile.risk_tolerance * 0.25
        aggression_penalty = min(0.25, avg_hits * 0.025)
        self.steering_aggression = min(1.35, max(0.75, aggression_base - aggression_penalty))
