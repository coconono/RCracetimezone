...
def update_car_state(
    ...
    accel = throttle * (car.max_speed * 1.35) - brake * (car.max_speed * 1.45)
    ...
    drift_trigger = abs(steering) > 0.5 and abs(state.speed) > car.max_speed * 0.58 and grip < 0.45
    if drift_trigger:
    ...
    if not _car_is_on_racing_surface(state, car, track):
        ...
        # Damage-over-time while off track.
        offtrack_damage_per_sec = 22.0
        offtrack_tire_loss_per_sec = 22.0
        speed_wear_factor = min(2.5, abs(state.speed) / max(car.max_speed * 0.35, 1.0))
        state.damage = min(100.0, state.damage + offtrack_damage_per_sec * dt)
    ...
    state.tire_health = max(0.0, state.tire_health - (abs(state.speed) / max(car.max_speed, 1.0)) * dt * 1.5)
    state.fuel = max(0.0, state.fuel - (0.15 + abs(state.speed) * 0.0014) * dt)

    if state.damage >= 100.0 or state.fuel <= 0.0:
        state.state = "crashed"
        state.speed = 0.0
        state.vx = 0.0
        state.vy = 0.0
