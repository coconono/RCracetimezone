...
def autonomous_controls(
    ...
    centerline = _build_centerline(track)
    if len(centerline) < 2:
        return 0.0, 0.0, 0.0, "", "no_centerline"
    ...
    if not line_visible:
        # If the line is lost, rotate toward the nearest route direction until reacquired.
        recover_pt = route_points[(nearest_route_idx + route_forward_step) % len(route_points)]
    ...
    return throttle, brake, steering, brake_reason, coast_reason
...
def main() -> int:
    ...
    menus = [
        ...
        ("Race", ["Start Race", "Pause/Resume", "Reset Cars", "Increase Waypoint Density", "Decrease Waypoint Density", "Quit Race"]),
        ...
        if racing and track is not None and sim_cars:
            ...
            for idx, entry in enumerate(sim_cars):
                ...
                if autonomous_enabled:
                    throttle, brake, steering, brake_reason, coast_reason = autonomous_controls(
                        state,
                        car,
                        track,
                        entry.race_elapsed,
                        ...
                        entry.route_stall_recover_time > 0.0,
                        entry.hard_route_recenter_time > 0.0,
                        entry.post_waypoint_boost_time,
                    )
